# FlareDataGrid (DataGrid)

**Category**: DataDisplay
**Tags**: data, grid, table, sort, filter, pagination, virtualization
**API Reference**: [https://jrfrigat.github.io/Flare/api/FlareDataGrid.html](https://jrfrigat.github.io/Flare/api/FlareDataGrid.html)
...
| `Filterable` | `bool` | No | `false` | Enable per-column filtering |
| `Sortable` | `bool` | No | `false` | Enable column sorting |
| `Pageable` | `bool` | No | `false` | Enable pagination |
| `PageSize` | `int` | No | `10` | Number of rows per page |
| `Virtualize` | `bool` | No | `false` | Enable row virtualization for large datasets |
| `Groupable` | `bool` | No | `false` | Enable column grouping |
| `Selectable` | `bool` | No | `false` | Enable row selection |
| `EditMode` | `DataGridEditMode` | No | `None` | Editing mode (Inline or Batch) |
| `Resizable` | `bool` | No | `false` | Enable column resizing |
| `Reorderable` | `bool` | No | `false` | Enable column reordering |
| `MaxHeight` | `string` | No | `` | Maximum height with scrolling |
| `EmptyTemplate` | `RenderFragment` | No | `` | Content shown when no items |
| `Loading` | `bool` | No | `false` | Shows loading state |

## Events

| Name | Type | Description |
|------|------|-------------|
| `OnRowClick` | `EventCallback<TItem>` | Fired when a row is clicked |
| `OnPageChanged` | `EventCallback<int>` | Fired when page changes |
| `OnSelectionChanged` | `EventCallback<IReadOnlyCollection<TItem>>` | Fired when row selection changes |
